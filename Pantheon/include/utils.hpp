...
#include "seal/modulus.h" // Add this if ConstModulusIter is defined here
...
void my_add_inplace(SEALContext &context_, Ciphertext &encrypted1, Ciphertext &encrypted2,Ciphertext &result);
...
// void apply_fermat_little_theorem(
//     Evaluator &evaluator,
//     Encryptor &encryptor,  // Added Encryptor
...
// void bfv_multiply(*context, column_results[0], temp_ct, MemoryManager::GetPool(), TOTAL_MACHINE_THREAD / NUM_ROW_THREAD);
...
void my_custom_add_inplace(SEALContext &context_, Ciphertext &encrypted1, Plaintext plainRep);
...
void bfv_multiply(SEALContext &context_,Evaluator &evaluator ,Ciphertext &encrypted1, Ciphertext &encrypted2,RelinKeys &relin_keys);
...
void my_custom_add_inplace(SEALContext &context_, Ciphertext &encrypted_matrix, Plaintext plainRep,Ciphertext &expanded_result){
    Evaluator evaluator(context_);  
    evaluator.add_plain(encrypted_matrix, plainRep, expanded_result);
...
void my_add_inplace(SEALContext &context_, Ciphertext &encrypted1, Ciphertext &encrypted2, Ciphertext &result)
{
    // Check if parms_id is valid for the given context
    if (!context_.get_context_data(encrypted1.parms_id()))
    {
        throw std::invalid_argument("encrypted1 has an invalid parms_id for the given SEALContext.");
    }

    if (!context_.get_context_data(encrypted2.parms_id()))
    {
        throw std::invalid_argument("encrypted2 has an invalid parms_id for the given SEALContext.");
    }

    // Ensure they are on the same modulus level
    if (encrypted1.parms_id() != encrypted2.parms_id())
    {
        Evaluator evaluator(context_);
        evaluator.mod_switch_to_inplace(encrypted1, encrypted2.parms_id());
    }
...
void my_fastbconv_m_tilde(const RNSTool *rns_tool, ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool, int num_threads)
{
    ...
    */
    ...
    my_fast_convert_array(rns_tool->base_q_to_Bsk_conv_, temp, destination, pool, num_threads);

    // Finally convert to {m_tilde}
    //rns_tool->base_q_to_m_tilde_conv()->fast_convert_array(temp, destination + base_Bsk_size, pool);
    my_fast_convert_array(rns_tool->base_q_to_m_tilde_conv_, temp, destination + base_Bsk_size, pool, num_threads);
}
...
//     // Step 1: Create a plaintext containing all 1s
//     Plaintext one_plain;
//     batch_encoder.encode(std::vector<uint64_t>(slot_count, 1ULL), one_plain);

//     // Step 2: Repeated squaring to compute a^(p-1) (i.e., a^16 where p = 17)
//     // This means we need to perform four rounds of squaring to get a^16.
//     for (int i = 0; i < 4; ++i) {
//         evaluator.square_inplace(encrypted);  // a^2, a^4, a^8, a^16
//         evaluator.relinearize_inplace(encrypted, relin_keys, pool);
//     }

//     // Step 3: Use subtraction to get the final value:
//     // 1 - encrypted to ensure all non-zero are turned into 1.
//     evaluator.sub_plain_inplace(encrypted, one_plain);

//     // Step 4: Multiply by -1 to flip the result:
//     // This step will turn `-1` into `1` and `0` remains `0`.
//     Plaintext minus_one_plain;
//     batch_encoder.encode(std::vector<uint64_t>(slot_count, PLAIN_MODULUS - 1), minus_one_plain);
//     evaluator.multiply_plain_inplace(encrypted, minus_one_plain);
// }
...
//cout<<"Pre time: "<<(chrono::duration_cast<chrono::microseconds>(time_end - time_start)).count()<<endl;
        ...
        for(int j = 0; j < encrypted_size; j++) {
            ...
            my_fast_convert_array(rns_tool->base_q_to_m_tilde_conv_, temp2, temp, pool, num_threads);
            my_fast_convert_array(rns_tool->base_q_to_m_tilde_conv_, temp2, temp + base_Bsk_size, pool, num_threads);
            ...
            SEAL_ALLOCATE_GET_COEFF_ITER(r_m_tilde, rns_tool->coeff_count_, pool);
            multiply_poly_scalar_coeffmod( input_m_tilde, rns_tool->coeff_count_, rns_tool->neg_inv_prod_q_mod_m_tilde_, rns_tool->m_tilde(), r_m_tilde);

            #pragma omp parallel for    
            for(int i = 0; i < base_Bsk_size; i++) {
                MultiplyUIntModOperand prod_q_mod_Bsk_elt;
                prod_q_mod_Bsk_elt.set(rns_tool->prod_q_mod_Bsk_[i], rns_tool->base_Bsk()->base()[i]);
                SEAL_ITERATE(iter(temp[i], r_m_tilde, encrypted_Bsk[j][i]), rns_tool->coeff_count_, [&](auto J) {
                    ...
                    get<2>(J) = multiply_uint_mod(
                        multiply_add_uint_mod(temp, prod_q_mod_Bsk_elt, get<0>(J),rns_tool->base_Bsk()->base()[i]), rns_tool->inv_m_tilde_mod_Bsk_[i],
...
void my_fast_convert_array(const Pointer<BaseConverter> &conv, ConstRNSIter in, RNSIter out, MemoryPoolHandle pool, int num_threads)
{
    ...
    conv->fast_convert_array(in, out, pool);
    #pragma omp parallel for collapse(2)
    for(int i = 0; i < obase_size; i++) {
        for(int j = 0; j < count; j++){
            out[i][j] = dot_product_mod(temp[j], out[i], ibase_size, obase_.base()[i]);
...
void my_sm_mrq(const RNSTool *rns_tool, ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool, int num_threads)
{
    ...
    */
    ...
    auto coeff_count_ = rns_tool->coeff_count_;
    // Compute r_m_tilde
    SEAL_ALLOCATE_GET_COEFF_ITER(r_m_tilde, coeff_count_, pool);
    multiply_poly_scalar_coeffmod( input_m_tilde, coeff_count_, rns_tool->neg_inv_prod_q_mod_m_tilde_, rns_tool->m_tilde(), r_m_tilde);
...
void my_fast_floor(const RNSTool *rns_tool, ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool, int num_threads)
{
    ...
    // Convert q -> Bsk
    my_fast_convert_array(rns_tool->base_q_to_Bsk_conv_,input, destination, pool, num_threads);

    // Move input pointer to past the base q components
    input += base_q_size;
    #pragma omp parallel for collapse(2) num_threads(num_threads)
    for(int i = 0; i < base_Bsk_size;i++) {
        for(int j = 0; j < rns_tool->coeff_count_; j++) {
            destination[i][j] = multiply_uint_mod(input[i][j] + (rns_tool->base_Bsk()->base()[i].value() - destination[i][j]), rns_tool->inv_prod_q_mod_Bsk_[i], rns_tool->base_Bsk()->base()[i]);
        }    
    }
}
...
void my_fastbconv_sk(const RNSTool *rns_tool, ConstRNSIter input, RNSIter destination, MemoryPoolHandle pool, int num_threads)
{
    ...
    */
    ...
    my_fast_convert_array(rns_tool->base_B_to_q_conv_, input, destination, pool, num_threads);


    // Compute alpha_sk
    // Fast convert B -> {m_sk}; input is in Bsk but we only use B
    SEAL_ALLOCATE_GET_COEFF_ITER(temp, coeff_count_, pool);
    my_fast_convert_array(rns_tool->base_B_to_m_sk_conv_, input, RNSIter(temp, coeff_count_), pool, num_threads);
    ...
    for(int i = 0; i < coeff_count_;i++) {
         alpha_sk[i] = multiply_uint_mod(temp[i] + (rns_tool->m_sk().value() - input[base_B_size][i]), rns_tool->inv_prod_B_mod_m_sk_, rns_tool->m_sk());
    }
    ...
    for(int i = 0; i < base_q_size; i++) {
        ...
        prod_B_mod_q_elt.set(rns_tool->prod_B_mod_q_[i], rns_tool->base_q()->base()[i]);

        MultiplyUIntModOperand neg_prod_B_mod_q_elt;
        neg_prod_B_mod_q_elt.set(rns_tool->base_q()->base()[i].value() - rns_tool->prod_B_mod_q_[i], rns_tool->base_q()->base()[i]);
...
/*Modified: used evaluator.rotate_columns_inplace(encrypted, galois_keys) to rotate*/
void my_apply_galois_inplace(SEALContext context_, Ciphertext &encrypted, uint32_t galois_elt, const GaloisKeys &galois_keys, MemoryPoolHandle pool, int num_threads)
{
    // SEALContext and Evaluator setup
    auto &context_data = *context_.get_context_data(encrypted.parms_id());
    auto &parms = context_data.parms();
    auto &coeff_modulus = parms.coeff_modulus();
    size_t coeff_count = parms.poly_modulus_degree();
    ...
    // Evaluator instance for the context
    Evaluator evaluator(context_);

    // Perform column rotation using the Galois keys
    try {
        evaluator.rotate_columns_inplace(encrypted, galois_keys);
    } catch (const std::invalid_argument &e) {
        std::cerr << "Error during rotation: " << e.what() << std::endl;
        return;
    }

    // Call your custom switch key function to handle relinearization or further processing
    // Ciphertext temp(context_);
...
void bfv_multiply(SEALContext &context_,Evaluator &evaluator ,Ciphertext &encrypted1, Ciphertext &encrypted2, RelinKeys &relin_keys){
      ...
      // Optionally relinearize if needed and if relinearization keys are available
        // evaluator.relinearize_inplace(encrypted1, relin_keys);

        
        // Adjust the scale to prevent overflow
    encrypted1.scale() = encrypted1.scale() * encrypted2.scale() / context_.key_context_data()->parms().plain_modulus().value();
...
//         // Adjust the scale to prevent overflow
//         encrypted1.scale() = encrypted1.scale() * encrypted2.scale() / context.key_context_data()->parms().plain_modulus().value();
...
//     // Set up iterators for the bases
//     auto rns_tool = context_data.rns_tool();
//     auto base_q = iter(parms.coeff_modulus());
//     auto base_Bsk = iter(rns_tool->base_Bsk()->base());
...
//     SEAL_ALLOCATE_GET_POLY_ITER(encrypted1_Bsk, encrypted1_size, coeff_count, rns_tool->base_Bsk()->size(), pool);
//     SEAL_ALLOCATE_GET_POLY_ITER(encrypted2_q, encrypted2_size, coeff_count, base_q_size, pool);
//     SEAL_ALLOCATE_GET_POLY_ITER(encrypted2_Bsk, encrypted2_size, coeff_count, rns_tool->base_Bsk()->size(), pool);

//     // Step 1-3: Extend base and convert for encrypted1 and encrypted2
//   auto convert = [&](auto tuple) {
//     // Extract the components from the tuple
//     auto in_iter = get<0>(tuple);
//     auto out_q_iter = get<1>(tuple);
//     auto out_Bsk_iter = get<2>(tuple);

//     SEAL_ALLOCATE_GET_RNS_ITER(temp, coeff_count, base_q_size + rns_tool->base_Bsk_m_tilde()->size(), pool);

//     // Copy and convert the input polynomials
//     for (size_t i = 0; i < base_q_size; i++) {
//         set_uint(in_iter[i], coeff_count, out_q_iter[i]);
//     }
//     my_fast_convert_array(rns_tool->base_q_to_Bsk_conv_, in_iter, temp, pool, num_threads);

//     // Convert for base Bsk
//     for (size_t i = 0; i < rns_tool->base_Bsk()->size(); i++) {
//         set_uint(temp[i], coeff_count, out_Bsk_iter[i]);
//     }
// };

// // Use the updated lambda with SEAL_ITERATE
// SEAL_ITERATE(iter(encrypted1, encrypted1_q, encrypted1_Bsk), encrypted1_size, convert);
// SEAL_ITERATE(iter(encrypted2, encrypted2_q, encrypted2_Bsk), encrypted2_size, convert);

//     // Allocate temporary space for the output of step (4)
//     SEAL_ALLOCATE_ZERO_GET_POLY_ITER(temp_dest_q, dest_size, coeff_count, base_q_size, pool);
//     SEAL_ALLOCATE_ZERO_GET_POLY_ITER(temp_dest_Bsk, dest_size, coeff_count, rns_tool->base_Bsk()->size(), pool);

//     // Step 4: Dyadic multiplication for both base q and base Bsk
//     auto dyadic_multiply = [&](ConstPolyIter in1_iter, ConstPolyIter in2_iter, ConstModulusIter base_iter, size_t base_size, PolyIter out_iter) {
...
//     dyadic_multiply(encrypted1_q, encrypted2_q, base_q, base_q_size, temp_dest_q);
//     dyadic_multiply(encrypted1_Bsk, encrypted2_Bsk, base_Bsk, rns_tool->base_Bsk()->size(), temp_dest_Bsk);

//     // Step 5: Multiply by t, convert base, and store the result
//     auto encrypted_iter1 = PolyIter(encrypted1);
//     SEAL_ALLOCATE_GET_RNS_ITER(temp_q_Bsk, coeff_count, base_q_size + rns_tool->base_Bsk()->size(), pool);
//     SEAL_ALLOCATE_GET_RNS_ITER(temp_Bsk, coeff_count, rns_tool->base_Bsk()->size(), pool);

//     for (size_t i = 0; i < dest_size; i++) {
//         for (size_t j = 0; j < rns_tool->base_Bsk()->size(); j++) {
//             multiply_poly_scalar_coeffmod(temp_dest_Bsk[i][j], (temp_q_Bsk + base_q_size).poly_modulus_degree(), plain_modulus, base_Bsk[j], (temp_q_Bsk + base_q_size)[j]);
//             if (j < base_q_size) {
//                 multiply_poly_scalar_coeffmod(temp_dest_q[i][j], temp_q_Bsk.poly_modulus_degree(), plain_modulus, base_q[j], temp_q_Bsk[j]);
//             }
...
// void my_apply_galois_inplace(SEALContext context_, Ciphertext &encrypted, uint32_t galois_elt, const GaloisKeys &galois_keys, MemoryPoolHandle pool, int num_threads)
// {
//     // Verify the ciphertext metadata
//     if (!is_metadata_valid_for(encrypted, context_) || !is_buffer_valid(encrypted)) {
//         throw std::invalid_argument("Input ciphertext is not valid for the provided context.");
//     }

//     // Ensure Galois key exists for the desired element
//     if (!galois_keys.has_key(galois_elt)) {
//         cerr << "Galois key not present for the specified step: " << galois_elt << endl;
//         throw std::invalid_argument("Galois key not present.");
//     }

//     // Debug: Print ciphertext metadata
...
//     Evaluator evaluator(context_);
//     try {
//         evaluator.rotate_rows_inplace(encrypted, galois_elt, galois_keys);
//         cout << "Rotation applied successfully for step: " << galois_elt << endl;
//     } catch (const exception &e) {
//         cerr << "Error during rotation: " << e.what() << endl;
//     }
// }
